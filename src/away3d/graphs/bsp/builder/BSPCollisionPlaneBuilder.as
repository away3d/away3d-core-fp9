package away3d.graphs.bsp.builder
{
	...
	internal class BSPCollisionPlaneBuilder extends AbstractBuilderDecorator implements IBSPPortalProvider
	{
		...
		{
			_iterator = new VectorIterator(Array(portals));
		...
		public function createBevelPlanes(node : BSPNode, sourceNGons : Array, targetNGons : Array) : void
		{
			...
			var bevels : Array = node._bevelPlanes;
					...
					while (--j >= 0) {
						...
						if (a1*a2+b1*b2+c1*c2 < -BSPTree.EPSILON &&
							...
							if (isNaN(bevel.a) || isNaN(bevel.b) || isNaN(bevel.c) || isNaN(bevel.d)) {
								...
								if (!bevels) node._bevelPlanes = bevels = new Array();
		...
		private function contains(list : Array, bevel : Plane3D) : Boolean
		{
