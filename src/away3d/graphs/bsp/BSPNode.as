package away3d.graphs.bsp
{
	...
	public final class BSPNode extends EventDispatcher implements ITreeNode
	{
		...
		arcane var _visList : Array;		// indices of leafs visible from this leaf
...
//				addCollider(child);
  ...
  */
		...
		arcane function removeFace(face : Face) : void
		{
  		 ...
  		 * @private
  		 */
  		arcane function addFaces(faces : Array) : void
		{
		...
		{
			if (!_visList) _visList = new Array();
			_visList.push(index);
		}
		 ...
		 * @private
		 */
		arcane function gatherLeaves(leaves : Array) : void
		{
...
//		arcane var _tempMesh : Mesh;
		 ...
		 * @private
		 */
		arcane function build(faces : Array = null) : void
		{
		 ...
		 */
		private function getBestPlane(faces : Array) : void
		{
			var face : NGon;
			var len : int = faces.length;
		 ...
		 */
		private function gatherConvexPlanes(faces : Array) : Array
		{
		 ...
		 */
		private function solidify(faces : Array) : void
		{
		 ...
		 */
		private function addNGons(faces : Array) : void
		{
			var len : int = faces.length;
			var tris : Array;
		 ...
		 */
		private function getPlaneScore(candidate : Plane3D, faces : Array) : void
		{
			var score : Number;
		 ...
		 */
		private function constructChildren(bestPlane : Plane3D, faces : Array) : void
		{
			var classification : int;
			var face : NGon;
			var len : int = faces.length;
			...
			do {
				...
				if (classification == -2) { 
					...
					var splits : Array = face.split(_partitionPlane);
	 ...
	 */
 		...
 		arcane function generatePortals(rootNode : BSPNode) : Array
 		{
 			...
 			var posPortals : Array;
 			var finalPortals : Array;
 			var splits : Array;
 			var i : int;
				...
				i = posPortals.length;
 				while (--i >= 0) {
					splits = _negativeNode.splitPortalByChildren(posPortals[i], Plane3D.BACK);
 					if (splits) {
 						if (!finalPortals) finalPortals = new Array();
		...
		public function assignPortal(portal : BSPPortal) : void
 		{
 			if (!_portals) _portals = new Array();
		...
		{
			if (!_backPortals) _backPortals = new Array();
